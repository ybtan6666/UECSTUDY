...
export async function generateOrderNumber(prisma: any): Promise<string> {
  try {
    ...
    // If there's an error (e.g., field doesn't exist in database yet), 
    // use a timestamp-based fallback
    console.warn("Error generating order number, using fallback:", error.message)
    const timestamp = Date.now()
    const random = Math.floor(Math.random() * 1000)
    return `ORD-${String(timestamp).slice(-6)}-${String(random).padStart(3, "0")}`
  }
}
